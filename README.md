...
앱에 들어가면 먼저 **프로젝트 목록**이 나옵니다. "+ 새 촬영 코디 만들기"로 프로젝트 이름과 촬영일자를 입력하면 새 프로젝트가 만들어져요. 각 프로젝트는 수정·삭제가 가능합니다.

촬영회차 안에 들어가면 바로 착장표가 나옵니다:
1. "아이템" 컬럼이 가로로 5개씩 나열되고, "+ 아이템 추가"로 얼마든지 늘릴 수 있어요. 컬럼 제목은 클릭해서 바로 수정 가능합니다.
2. 왼쪽 사이드바에서 제품명 검색 · 시즌/카테고리 칩 필터로 제품을 찾고, 썸네일을 각 모델×아이템 셀 위로 드래그해서 놓으면 담깁니다. 담긴 제품마다 이름·사이즈 입력칸·수령완료 체크박스·입고일이 함께 표시됩니다.
3. **표 저장**으로 조합표 전체를 저장하고, "저장된 조합표" 탭에서 다시 불러오거나 삭제할 수 있습니다.

> 모델 선택 기능은 현재 화면에서 빠져 있어요(제품 위주로 빠르게 쓰기 위해 잠시 꺼둔 상태). 노션 모델DB 연동과 관련 API는 코드에 그대로 남아있어서, 나중에 다시 켜고 싶다고 말씀해주시면 바로 복원해드릴 수 있어요.
